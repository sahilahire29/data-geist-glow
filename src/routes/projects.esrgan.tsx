  ...
  head: () => ({
    meta: [
      { title: "ESRGAN Image Enhancer — Sahil Ahire" },
      {
        name: "description",
        content:
          "Architecture and results of an ESRGAN single-image super-resolution pipeline for detailed 4× image enhancement.",
      },
      { property: "og:title", content: "Image Resolution Enhancer using ESRGAN" },
      {
        property: "og:description",
        content: "RRDB-based super-resolution with perceptual, adversarial, and content losses.",
      },
      ...
      <div className="grid md:grid-cols-2">
        <figure className="border-b border-border md:border-b-0 md:border-r">
          <div className="aspect-[3/2] overflow-hidden bg-background">
            <img
              src="/low_res.png"
              alt="Pixelated low-resolution architectural input demonstration"
              width={192}
              height={128}
              loading="lazy"
          ...
          <figcaption className="flex items-center justify-between gap-3 p-4 text-xs">
            <span className="font-mono text-muted-foreground">INPUT / 1×</span>
            <span className="text-muted-foreground">Soft edges, lost texture</span>
          </figcaption>
        </figure>
        <figure>
          <div className="aspect-[3/2] overflow-hidden bg-background">
            <img
              src="/high_res.png"
              alt="Sharp high-resolution architectural output demonstration"
              width={1536}
              height={1024}
              loading="lazy"
          ...
          <figcaption className="flex items-center justify-between gap-3 p-4 text-xs">
            <span className="font-mono text-primary">ESRGAN / 4×</span>
            <span className="text-muted-foreground">Recovered edges and texture</span>
          </figcaption>
        </figure>
      </div>
      <p className="border-t border-border px-5 py-3 text-xs leading-5 text-muted-foreground">
        Generated demonstration example. Replace with a captured model output when final benchmark
...
function EsrganPage() {
  return (
    <ProjectDetail
      ...
      results={[
        { value: "4×+", label: "Resolution scale" },
        { value: "PSNR", label: "Signal quality" },
        { value: "SSIM", label: "Structural fidelity" },
        { value: "LPIPS", label: "Perceptual quality" },
      ]}
      architecture={[
        {
          label: "Prepare",
          detail:
            "Create paired low- and high-resolution samples with consistent crops and augmentation.",
        },
        {
          label: "Generate",
          detail:
            "Pass the input through RRDB layers without batch normalization to reconstruct fine detail.",
        },
        {
          label: "Optimize",
          detail:
            "Combine perceptual, adversarial, and content losses for fidelity and realistic texture.",
        },
        {
          label: "Enhance",
          detail:
            "Run tiled inference and blend boundaries to process large images within available memory.",
        },
      ]}
    ...
    >
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-md border border-border bg-card/45 p-4">
          ...
          <h3 className="mt-4 text-sm font-semibold">Natural texture</h3>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            VGG-based perceptual loss avoids the overly smooth finish produced by pixel loss alone.
          </p>
        </div>
        <div className="rounded-md border border-border bg-card/45 p-4">
          <ScanSearch size={17} className="text-accent" />
          <h3 className="mt-4 text-sm font-semibold">Memory-aware inference</h3>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Seamless tiling supports ultra-high-resolution outputs on consumer hardware.
          </p>
        </div>
      </div>
    </ProjectDetail>
