      ...
      <main className="mx-auto max-w-7xl px-5 pb-20 pt-28 sm:px-8 sm:pt-36">
        ...
        <div className="grid gap-5 py-8 sm:grid-cols-2 lg:grid-cols-4">
          {results.map((result, i) => (
            <Reveal key={result.label} delay={i * 0.06}>
            ...
            <section>
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                {stack.map((item) => (
                  <span
                    key={item}
                    className="rounded-sm border border-border bg-card/70 px-3 py-1.5 font-mono text-xs text-muted-foreground"
                  >
                    {item}
                  </span>
                ))}
          ...
          <section className="border-y border-border py-12">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {architecture.map((step, i) => (
                <div
                  key={step.label}
                  className="group relative overflow-hidden rounded-md border border-border bg-card/55 p-5"
                >
        ...
        <div className="mt-16 flex flex-col items-start justify-between gap-6 border-b border-border pb-12 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold sm:text-3xl">{nextLabel}</h2>
          </div>
          <Link
            to={nextTo}
            className="inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-transform hover:translate-x-1"
          >
